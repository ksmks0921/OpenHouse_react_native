...
class CreateAccountThreeScreen extends Component {
    ...
    async componentDidMount() {
        console.log("================== CreateAccountThreeScreen & brokersname  ==================");

        this.props.navigation.setParams({ addnewtitle: this._addnewtitle });
        let self=this;
        axios_instance
        .get(`${Constants.GET_BASE_API_URL}?deviceid=${deviceid}&table=${"brokersname"}`)
        .then(res => {
          if (res.status === 200) {
            self.setState({createaccount : res.data});
            console.log("brokersname ==============>", res.data);
    ...
    itemClick=(item)=>{
        if(item && item.company_name){
            ...
            Constants.officetelephone = item.company_telephone;
            Constants.company_logo_url = item.company_logo_url;
            console.log("CreateAccountThreeScreen", item.company_telephone, item.company_logo_url);
